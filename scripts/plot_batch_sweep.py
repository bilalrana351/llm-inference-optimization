...
def _median_by_batch(df: pd.DataFrame) -> pd.DataFrame:
    """Collapse repeats to the median at each (pool_config, batch_size)."""
    num_cols = [
        "throughput_out_tok_s", "decode_throughput_tok_s", "tpot_ms",
        "ttft_s", "p50_latency_s", "p95_latency_s", "kv_capacity_seqs",
    ]
    keep = ["pool_config", "batch_size"] + [c for c in num_cols if c in df.columns]
    g = df[keep].groupby(["pool_config", "batch_size"], as_index=False).median()
    return g.sort_values(["pool_config", "batch_size"])


def _kv_wall(df: pd.DataFrame, config: str) -> float | None:
    """The analytical KV wall (sequences) for a pool config, if vLLM exposed it."""
    vals = df.loc[df["pool_config"] == config, "kv_capacity_seqs"]
    vals = vals[vals > 0]
...
def plot_throughput(df: pd.DataFrame, out_path: str) -> None:
    ...
    for config, sub in df.groupby("pool_config"):
        ax.plot(sub["batch_size"], sub["throughput_out_tok_s"],
                marker="o", label=f"{config} pool")
        wall = _kv_wall(df, config)
        if wall:
    ...
    ax.set_ylabel("end-to-end throughput (output tok/s)")
    ax.set_title("vLLM throughput vs batch size (Qwen2.5-1.5B, RTX 3060)")
...
def plot_latency(df: pd.DataFrame, out_path: str) -> None:
    fig, ax = plt.subplots(figsize=(8, 5))
    for config, sub in df.groupby("pool_config"):
        ax.plot(sub["batch_size"], sub["tpot_ms"],
                marker="o", label=f"{config}: TPOT")
        if "p95_latency_s" in sub and sub["p95_latency_s"].notna().any():
            ax.plot(sub["batch_size"], sub["p95_latency_s"] * 1000,
                    marker="x", linestyle=":", label=f"{config}: p95 end-to-end")
    ax.set_xscale("log", base=2)
    ax.set_xlabel("concurrent sequences (batch size)")
...
def plot_tradeoff(df: pd.DataFrame, out_path: str) -> None:
    fig, ax = plt.subplots(figsize=(8, 5))
    for config, sub in df.groupby("pool_config"):
        ax.plot(sub["tpot_ms"], sub["throughput_out_tok_s"],
                marker="o", label=f"{config} pool")
        for _, row in sub.iterrows():
            ax.annotate(f"{int(row['batch_size'])}",
                        (row["tpot_ms"], row["throughput_out_tok_s"]),
